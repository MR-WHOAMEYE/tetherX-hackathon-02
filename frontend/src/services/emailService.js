...
/**
 * Send an OTP verification email to a patient via EmailJS.
 *
 * @param {Object} params
 * @param {string} params.patientName  - Patient's display name
 * @param {string} params.patientEmail - Patient's email address
 ...
 * @returns {Promise} EmailJS send result
 */
export async function sendOtpEmail({ patientName, patientEmail, otp }) {
    if (!SERVICE_ID || !OTP_TEMPLATE_ID || !PUBLIC_KEY) {
        console.warn('[EmailJS] Missing OTP config — email not sent. Check VITE_EMAILJS_OTP_TEMPLATE_ID in .env');
        return null;
    }
    ...
    const templateParams = {
        ...
        to_email: patientEmail,
        otp_code: otp,
        subject: 'Your TetherX Verification Code',
        from_name: 'TetherX Health',
    };

    const result = await emailjs.send(SERVICE_ID, OTP_TEMPLATE_ID, templateParams, PUBLIC_KEY);
    console.log('[EmailJS] OTP email sent:', result.status);
    return result;
}
